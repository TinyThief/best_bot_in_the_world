...
| Telegram /chart | `_run_candlestick_chart(..., limit=730)` через db_helper.ensure_fresh_then_get | Передаёт в build_candlestick_trend_chart список **[oldest … newest]** (последние 2 года) | ✓ |
| Срез | `candles[-max_candles_display:]` при n_total > 730 | Берём **последние** 730 элементов (2 года), порядок сохранён | ✓ |
...
- **В build_candlestick_trend_chart (BTC):** отбрасываются свечи, у которых хотя бы одно значение OHLC **вне** 1_000–150_000 USDT или (high-low)/open > 30%. Остальное — как в данных.
- **В exchange._filter_valid_ohlc:** при загрузке с биржи отсекаются свечи вне диапазона 1k–150k USDT и с абсурдным (high-low)/open > 30%.
...
|------|-----------------------------|
| В БД записаны старые данные | «Последняя свеча» будет с прошлой даты. Решение: `python bin/catch_up_db.py` или `python bin/refresh_db.py --yes`. |
...
| Цены > 150k (BTC) | Фильтр в графике и в exchange отсекает — такие свечи не попадают в БД/график. Лимит задан в exchange.py (1k–150k USDT). |
...
2. **Фильтр в графике:** проверка цен 1k–150k USDT для BTC и (high-low)/open ≤ 30%.
...
- При расхождении цен с TradingView: проверить актуальность БД (`python bin/catch_up_db.py` / `python bin/refresh_db.py` / `python bin/fill_gap_db.py`) и лимиты фильтра цен (1k–150k USDT).
