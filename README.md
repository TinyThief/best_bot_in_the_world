...
Нужен токен от [@BotFather](https://t.me/BotFather): создай бота, вставь токен в `.env` как `TELEGRAM_BOT_TOKEN`. Команды: `/start`, `/signal` — полный разбор (сигнал, фазы по ТФ, зоны, импульс, score), `/status` — краткий статус одной строкой, `/zones` — торговые зоны (поддержка/сопротивление, перевороты, confluence), `/momentum` — импульс по старшему ТФ (RSI, состояние, направление), `/db` — статистика БД, `/health` — свежесть БД по ТФ, `/chart` — свечной график (ТФ D, из БД), `/phases` — график 6 фаз рынка, `/trend_daily` — тренд по всей БД ТФ D (зоны Вверх/Вниз/Флэт), `/backtest_phases` — график бэктеста фаз, `/id` — твой user id для TELEGRAM_ALLOWED_IDS, `/help`. Под ответами — кнопки **Сигнал | Зоны | Импульс** и Обновить | БД. Алерт при смене сигнала: в `.env` задай `TELEGRAM_ALERT_CHAT_ID=<chat_id>`, `TELEGRAM_ALERT_ON_SIGNAL_CHANGE=1`. Ограничение доступа: `TELEGRAM_ALLOWED_IDS=123,456` в `.env`.
