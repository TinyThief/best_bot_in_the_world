...
- **Тики** — CSV по дням в `data/history/trades/{SYMBOL}/{YEAR}/` (папка на год). Загрузка: `python bin/download_history.py --download --from YYYY-MM-DD --to YYYY-MM-DD`. Раскладка по годам: `python bin/download_history.py --organize-by-year`.
...
- Тики и бэктест песочницы → загрузка тиков: `bin/download_history.py --download --from/--to`; бэктест: `bin/backtest_sandbox.py --from/--to` (повтор не гоняется без `--force`); отчёт: `bin/sandbox_backtest_report.py --year YYYY` или `--all`. Подробнее — bin/README.md.
