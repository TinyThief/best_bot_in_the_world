...
**Карта проекта (всё по полочкам):** [STRUCTURE.md](STRUCTURE.md) — дерево, таблицы по назначению, где что искать.
...
- **src/core/** — инфраструктура: конфиг (`config.py`), БД (`database.py`), биржа Bybit (`exchange.py`), умные выборки по БД (`db_helper.py` — is_stale, catch_up_tf, get_candles_last_days, ensure_fresh_then_get, дозаполнение пропусков), логирование (`logging_config.py`).
- **src/analysis/** — аналитика: фазы рынка (`market_phases.py` — основной, Вайкофф + индикаторы, свинг-уровни), тренд и режим рынка (`market_trend.py` — detect_trend, detect_regime, detect_momentum), **торговые зоны** (`trading_zones.py` — уровни, перевороты ролей, confluence по ТФ), мультитаймфреймовый разбор и сигнал (`multi_tf.py` — параллельный расчёт по ТФ, зоны по старшему ТФ). Экспериментальные модули фаз: только Вайкофф (`phase_wyckoff.py`), только индикаторы (`phase_indicators.py`), только price action (`phase_structure.py`).
...
- **Корень** — главные точки входа: `main.py`, `telegram_bot.py`, `check_all.py`, `release.py`; конфиг и документация. Скрипты БД и бэктестов — в **bin/** (запуск из корня: `python bin/catch_up_db.py`, `python bin/accumulate_db.py` и т.д.; см. bin/README.md).
...
Запуск из корня: `python main.py`, `python telegram_bot.py`, `python bin/accumulate_db.py`, `python bin/backtest_phases.py` и т.д. (список — в bin/README.md).
...
- **`_filter_valid_ohlc(...)`** — отсев свечей с нереалистичными ценами (для BTCUSDT: 1000–150 000 USDT) и с абсурдным интрадей-диапазоном (high-low)/open > 30%. Применяется ко всем ответам get_klines и fetch_klines_backfill.
...
### `src/core/db_helper.py`

**Умные выборки по БД.** Используется для графика `/chart` и везде, где нужны актуальные данные без полной перезаливки БД.

- **`is_stale(conn, symbol, timeframe, max_lag_sec)`** — True, если последняя свеча в БД старше max_lag_sec (по умолчанию 1 сутки).
- **`catch_up_tf(conn, symbol, timeframe)`** — догружает один ТФ с момента последней свечи до «сейчас»; возвращает число вставленных свечей.
- **`get_candles_last_days(conn, symbol, timeframe, days, use_cache)`** — свечи за последние N дней (от последней в БД); кэш 60 с по (symbol, tf, days).
- **`ensure_fresh_then_get(conn, symbol, timeframe, days, max_lag_sec, use_cache)`** — при устаревших данных догружает ТФ; при малом числе свечей (<60% от days) один раз дозаполняет пропуски через `run_fill_gap_for_timeframe` и перезапрашивает; возвращает список свечей.

Лимит выборки для «последние N дней» — до 2000 свечей (для 730 дней запрашивается до 930).

---

### `src/scripts/accumulate_db.py`

**Скрипт, который наполняет и поддерживает базу свечей.** Точка входа для «накопления данных под обучение». Лаунчер: `python bin/accumulate_db.py`.
...
- **`run_extend_until_done(conn)`** — по каждому ТФ подгружает историю вглубь от самой старой свечи, пока есть новые данные; используется из `db_sync.open_and_prepare()` при `AUTO_EXTEND_AT_STARTUP=1`, чтобы по всем ТФ (в т.ч. 3m) глубина нарастала автоматически без ручного `python bin/full_backfill.py --extend`.
- **`run_fill_gap_for_timeframe(cursor, symbol, timeframe)`** — дозаполняет пропуски между самой старой и самой новой свечой по этому ТФ (запрашивает у биржи недостающий диапазон и вставляет только отсутствующие свечи). Используется из `db_helper.ensure_fresh_then_get` при малом числе свечей и из лаунчера `python bin/fill_gap_db.py`.
...
Запуск: `python bin/accumulate_db.py`. Остановка — Ctrl+C.
...
- **`analyze_multi_timeframe(...)`** — запрашивает свечи по всем заданным таймфреймам (из БД или биржи), для каждого ТФ считает тренд (`detect_trend`), режим рынка (`detect_regime`: тренд/диапазон/всплеск), фазу (`detect_phase`), устойчивость фазы и тренда; по старшему ТФ — фильтры (объём, ATR, расстояние до свинг-уровней, совпадение ТФ по тренду/фазе, режим «всплеск»). Сигнал long/short выдаётся только при `phase_decision_ready` (фаза ясна и устойчива, тренд ясен, фильтры пройдены, режим не «всплеск», качество свечей ок при `CANDLE_QUALITY_MIN_SCORE > 0`). Единый score входа (0..1) — взвешенная сумма фазы, тренда и доли совпадающих ТФ; `confidence` = entry_score при наличии направления.
  - `higher_tf_trend`, `higher_tf_regime`, `higher_tf_phase`, импульс по старшему ТФ (momentum_state, RSI, return_5)
  - `trading_zones` — зона low/high, nearest_support/resistance, recent_flips, levels_with_confluence
...
Расчёт по каждому ТФ (quality, trend, phase без контекста, regime, momentum) выполняется **параллельно** (ThreadPoolExecutor); обновление истории устойчивости, контекст старшего ТФ и зоны — последовательно. Использует настройки `SYMBOL` и `TIMEFRAMES` из конфига. При `DATA_SOURCE=db` и переданном соединении с БД читает свечи из локальной БД; при `DATA_SOURCE=exchange` — запрос к Bybit на каждый вызов.
...
### Сравнение методов фаз: `src/scripts/compare_phase_methods.py` и лаунчер `bin/compare_phase_methods.py`

**Сравнение трёх способов определения фазы:** только Вайкофф (`phase_wyckoff`), только индикаторы (`phase_indicators`), только price action / BOS-CHOCH (`phase_structure`). Один и тот же бэктест по БД — вывод точности по направлению для каждого метода. Запуск: `python bin/compare_phase_methods.py [--tf 60] [--bars 20000] [--step 5]`. Для продакшена используется основной `market_phases.py` (Вайкофф + индикаторы).

### Бэктест фаз: `src/scripts/backtest_phases.py` и лаунчер `bin/backtest_phases.py`

**Проверка точности фаз по истории из БД.** Скользящее окно → `detect_phase` → форвард-доходность; «точность по направлению». Запуск: `python bin/backtest_phases.py [--tf 60] [--bars 20000] [--forward 20] [--step 5]`. **`--min-score 0.6`** — только бары с score ≥ 0.6. **`--tune`** — подбор порогов.

### `bin/refresh_db.py`

**Полное обновление БД:** удаление файла БД и загрузка всех таймфреймов из `TIMEFRAMES_DB` с биржи. Запуск: `python bin/refresh_db.py [--yes]`. Перед запуском останови бота. Без подтверждения: `--yes`.

### `bin/refill_tf_d.py`

**Перезалив только ТФ D:** удаление всех свечей по паре и ТФ D, затем загрузка истории заново с Bybit (с фильтром нереалистичных цен). Запуск: `python bin/refill_tf_d.py`. Используется, если по ТФ D попали некорректные цены.

### `bin/catch_up_db.py`

**Догрузка БД до текущей даты:** подтягивает пропущенные свечи от последней в БД до «сейчас» по всем ТФ из `TIMEFRAMES_DB`. Запуск: `python bin/catch_up_db.py`. Выводит дату последней свечи (ТФ D и 60).

### `src/scripts/full_backfill.py` и лаунчер `bin/full_backfill.py`

**Полный бэкфилл за весь период Bybit.** Запуск: `python bin/full_backfill.py [--clear] [--extend]`.
...
### `src/scripts/test_run_once.py` и лаунчер `bin/test_run_once.py`

**Один прогон анализа для теста.** Запуск: `python bin/test_run_once.py`.

### Исторические тики и бэктест песочницы микроструктуры

- **Тики** — CSV по дням в `data/history/trades/{SYMBOL}/{YEAR}/` (папка на год). Загрузка: `python bin/download_history.py --download --from YYYY-MM-DD --to YYYY-MM-DD`. Подгрузка недостающих дат (public.bybit.com + REST за сегодня): `python bin/refill_ticks.py --from YYYY-MM-DD --to YYYY-MM-DD`. Раскладка по годам: `python bin/download_history.py --organize-by-year`.
- **Бэктест песочницы** — replay тиков, синтетический стакан: `python bin/backtest_sandbox.py --from YYYY-MM-DD --to YYYY-MM-DD`. Пройденные диапазоны сохраняются в `logs/sandbox_backtest_completed.json`; повторный запуск того же периода пропускается (`--force` — принудительно, `--mark-done` — только записать диапазон как пройденный).
- **Отчёт по бэктесту** — PnL и разбивка по причинам выхода: `python bin/sandbox_backtest_report.py --year 2025`; сводка за несколько лет: `python bin/sandbox_backtest_report.py --all`. Полный список команд — в **bin/README.md**.
...
Команды: `/start` — старт и нижняя панель; `/signal` — полный разбор (сигнал, фазы по ТФ, зоны, импульс, entry_score_breakdown); `/status` — одна строка; `/zones` — торговые зоны (поддержка/сопротивление, перевороты, confluence); `/momentum` — импульс по старшему ТФ (состояние, RSI, направление); `/db` — статистика БД; `/health` — свежесть БД по ТФ (последняя свеча по каждому ТФ); `/chart`, `/phases`, `/trend_daily`, `/backtest_phases` — графики; `/id` — user id для TELEGRAM_ALLOWED_IDS; `/help` — справка (HTML-разметка). Под ответами «Сигнал», «Зоны», «Импульс» — inline-кнопки **Сигнал | Зоны | Импульс** и Обновить | БД. **Алерт при смене сигнала:** задай в .env `TELEGRAM_ALERT_CHAT_ID=<chat_id>`, `TELEGRAM_ALERT_ON_SIGNAL_CHANGE=1`; job раз в TELEGRAM_ALERT_INTERVAL_SEC проверяет direction и при смене шлёт сообщение в указанный чат. Ограничение доступа: `TELEGRAM_ALLOWED_IDS=123,456` в .env.
...
| `TELEGRAM_ALERT_CHAT_ID` | Чат id для алертов при смене сигнала (пусто = выкл) | — |
| `TELEGRAM_ALERT_ON_SIGNAL_CHANGE` | 1 = включить проверку смены direction и отправку в TELEGRAM_ALERT_CHAT_ID | `0` |
| `TELEGRAM_ALERT_INTERVAL_SEC` | Интервал проверки (сек) | `90` |
| `TELEGRAM_ALERT_MIN_CONFIDENCE` | Мин. уверенность для алерта (0 = любая) | `0` |
...
- Проверить точность фаз по истории → `python bin/backtest_phases.py [--tf 60] [--min-score 0.6]`; подбор порогов — `--tune`. Проверить точность тренда → `python bin/backtest_trend.py [--tf 60]`. Сравнить методы фаз (Wyckoff / индикаторы / PA) → `python bin/compare_phase_methods.py [--tf 60]`.
- Тики и бэктест песочницы → загрузка тиков: `bin/download_history.py --download --from/--to`; подгрузка недостающих: `bin/refill_ticks.py --from/--to`; бэктест: `bin/backtest_sandbox.py --from/--to` (повтор не гоняется без `--force`); отчёт: `bin/sandbox_backtest_report.py --year YYYY` или `--all`. Подробнее — bin/README.md.
